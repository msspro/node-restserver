...
const { verificaToken, verificaRole } = require('../midlewares/autenticacion');
...
app.get('/usuario', verificaToken, (req, res) => {
    ...
    let _estado = req.query.estado || true;
    ...
    Usuario.find({ estado: _estado }, 'nombre email role estado google img')
        .skip(desde)
        .limit(limite)
        .exec((err, usuarios) => {
            if (err) {
            ...
            Usuario.count({ estado: _estado }, (err, conteo) => {
                res.json({
                    ok: true,
                    usuarios,
                    total: conteo
